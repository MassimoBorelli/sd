...
# dropping out the leverage point n[1]
shiftn = n-median(n) # n - 26
modelEpsilonSquared = lm (Y[2:49] ~ shiftn[2:49] + I(shiftn[2:49]^2))
summary(modelEpsilonSquared)
par(mfrow = c(2,2))
plot(modelEpsilonSquared)
par(mfrow = c(1,1))
# therefore it would be better to adopt: 
ourImprovedNewEpsilon = exp(n/(-9.0164688 -0.2323802 * shiftn + 0.0007355 * shiftn^2 ))
# instead of 'ourNewEpsilon'
cbind(eta_n, round(ourNewEta, 3), round(BlomEta + ourImprovedNewEpsilon, 3))
############################################################
